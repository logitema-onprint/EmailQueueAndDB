...
export const pauseSelectedOrders: RequestHandler = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const jobsResult = await QueueService.pauseOrders(existingOrderIds);

    const message =
      missingOrderIds.length > 0
        ? `Successfully pause: ${jobsResult.totalJobsProcessed} jobs and ${
            existingOrderIds.length
          } orders. Missing orders: ${missingOrderIds.join(", ")}`
        : `Successfully pause: ${jobsResult.totalJobsPaused} jobs and ${existingOrderIds.length} orders`;
    ...
    res.status(500).json({
      success: false,
      message: `Failed pause selected orders ${error}`,
    });
  }
};
